...
module under test is imported through importlib so the stubs take effect.
"""
...
def clean_env(monkeypatch: pytest.MonkeyPatch):
    """Install minimal astrbot stubs, then import core.clean fresh."""
    ...
    for mod_name in ("core.config", "core.clean"):
        monkeypatch.delitem(sys.modules, mod_name, raising=False)

    return SimpleNamespace(clean=importlib.import_module("core.clean"))


# 通过 __new__ 构造实例，跳过 AsyncIOScheduler 启动（测试只关心清理逻辑）
def _make_cleaner(clean_env, cfg: SimpleNamespace):
    cleaner = clean_env.clean.CacheCleaner.__new__(clean_env.clean.CacheCleaner)
    cleaner.cfg = cfg
    return cleaner
...
class TestRemoveExpired:
    def test_fresh_file_kept(self, clean_env, tmp_path: Path) -> None:
        _write(tmp_path / "fresh.mp4")
        removed = clean_env.clean.CacheCleaner._remove_expired(
            tmp_path, cutoff=time.time() - 1000
        )
        assert removed == 0
        assert (tmp_path / "fresh.mp4").exists()

    def test_expired_file_deleted(self, clean_env, tmp_path: Path) -> None:
    ...
    def test_mixed_expired_deleted_fresh_kept(self, clean_env, tmp_path: Path) -> None:
        _write(tmp_path / "old.mp4", age=10000)
        _write(tmp_path / "fresh.mp4")
        removed = clean_env.clean.CacheCleaner._remove_expired(
            tmp_path, cutoff=time.time() - 1000
    ...
    def test_cache_root_never_removed(self, clean_env, tmp_path: Path) -> None:
        _write(tmp_path / "old.mp4", age=10000)
        clean_env.clean.CacheCleaner._remove_expired(
            tmp_path, cutoff=time.time() - 1000
        )
    ...
    def test_nested_fresh_dir_kept(self, clean_env, tmp_path: Path) -> None:
...
class TestRaceTolerance:
    def test_missing_cache_dir_returns_zero(self, clean_env, tmp_path: Path) -> None:
        missing = tmp_path / "does-not-exist"
        assert (
            clean_env.clean.CacheCleaner._remove_expired(
                missing, cutoff=time.time() - 1000
            )
            == 0
        )

    def test_file_vanishing_mid_scan_tolerated(self, clean_env, tmp_path: Path) -> None:
...
class TestSchedulerEntry:
    def test_http_mode_property_true(self, clean_env) -> None:
        cleaner = _make_cleaner(clean_env, SimpleNamespace(media_send_mode="http"))
        assert cleaner.http_mode is True

    def test_local_mode_property_false(self, clean_env) -> None:
        cleaner = _make_cleaner(clean_env, SimpleNamespace(media_send_mode="local"))
        assert cleaner.http_mode is False

    def test_missing_mode_defaults_to_local(self, clean_env) -> None:
        cleaner = _make_cleaner(clean_env, SimpleNamespace(media_send_mode=None))
        assert cleaner.http_mode is False

    @pytest.mark.asyncio
    async def test_http_mode_ttl_clean_dispatch(
        self, clean_env, tmp_path: Path
    ) -> None:
        ...
        cleaner = _make_cleaner(clean_env, cfg)
        _write(tmp_path / "old.mp4", age=10000)
        _write(tmp_path / "fresh.mp4")
...
class TestLocalWholeClean:
    @pytest.mark.asyncio
    async def test_local_mode_clean_whole_recreates(
        self, clean_env, tmp_path: Path
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_local_mode_dispatch_not_ttl(self, clean_env, tmp_path: Path) -> None:
